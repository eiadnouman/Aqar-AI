    ...
    with st.container():
        ...
        st.markdown(f"""
            <div style="padding: 15px;">
                ...
                <div class="spec-row">
                    <span class="spec-item">🛏️ {int(meta.get('bedrooms', 0))} Beds</span>
                    <span class="spec-item">🚿 {int(meta.get('bathrooms', 0))} Baths</span>
                    <span class="spec-item">📐 {int(meta.get('size', 0))} sqm</span>
                </div>
                <div style="font-size:0.85rem; opacity: 0.7; height:45px; overflow:hidden; margin-bottom:10px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">
                    {doc.page_content.split('Description: ')[-1].splitlines()[0][:120]}
                </div>
                <a href="{meta.get('url', '#')}" target="_blank" class="view-link">🔗 View Details</a>
            </div>
        </div>
...
def main():
    ...
    with st.sidebar:
        ...
        if rag and rag.vectorstore:
            ...
            st.metric("Properties Live", f"{count:,}", delta="Active")
        else:
            st.error("Engine Not Connected")
